...
export async function optimizeMix(req: MixRequest): Promise<string> {
  const startState = new Set<string>(
    req.initialEffect ? [req.initialEffect] : [],
  );
  ...
  (globalThis.mixerResults as Map<string, OptimizationResult>).set(
    resultId,
    result,
  );
...
export async function getOptimizationResult(
  ...
  return (
    (globalThis.mixerResults as Map<string, OptimizationResult>).get(
      resultId,
    ) || null
  );
}
